using vanwall.crypto.coin.bitcoin;
using vanwall.crypto.coin.dogecoin;
using vanwall.crypto.coin.ethereum;
using vanwall.crypto.algorithms.secp256k1;
using vanwall.io;
...
namespace vanwall.crypto.coin
{
    ...
    public static class CoinCore
    {
        ...
        private static async Task StoreGeneratedKeysAsync(List<ICoin> coins, byte[] privateKeyBytes, byte[] publicKeyBytes)
        {
            ...
            {
                await deltaCoin.BufferKeyPairAsync(deltaCoin.GenerateAddressFromPublicKey(publicKeyBytes), privateKeyBytes);
            }
        }
